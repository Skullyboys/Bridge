...
const bscBridgeAbi = require("./abis/BscBridge.json");
const DoveAbi = require("./abis/BscDoveAbi.json")
const CROSS_CHAIN_ID = 56;
const CHAIN_ID = 137;
...
const CROSS_SWAP_ADDRESS = "0x9d0D21C2ede0a7B081B1E5FC8e307674b9c50199";
...
const web3Polygon = new Web3(new Web3.providers.HttpProvider("https://bsc-dataseed.binance.org/"));

const CROSS_SWAP_INSTANCE = new web3Polygon.eth.Contract(bscBridgeAbi, CROSS_SWAP_ADDRESS);
...
async function checkPending() {
    ...
    fs.readFile(path.resolve(__dirname, 'polygonBlock.json'), async (err, blockData) => {
        ...
        DOVE_INSTANCE.getPastEvents({},
            ...
            .then(async function (resp) {
                for (let i = 0; i < resp.length; i++) {
                    console.log("console -"+i ,resp[i]);
                    if (resp[i].event === "Transfer" && resp[i].returnValues.to == "0x6E07257B6C90ae1402a5B3Dc3D8db5C03122dB22") {
        ...
        fs.writeFile(path.resolve(__dirname, './polygonBlock.json'), JSON.stringify(blockData), (err) => {
            if (err);
            console.log(err);
        });
    });
